...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        dx_t, dh0, dWx_t, dWh_t, db_t = rnn_step_backward(dh0 + dh[:, t, :], cache[t])
        dx[:, t, :] = dx_t
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    di = dnext_c * g
    dg = dnext_c * i
    difog = np.concatenate([di * i * (1 - i), df * f * (1 - f), do * o * (1 - o), dg * (1 - g ** 2)], axis=1)
    dx = difog @ Wx.T
    dWx = x.T @ difog
    dprev_h = difog @ Wh.T
    dWh = prev_h.T @ difog
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    c0 = np.zeros_like(h0)
    for t in range(T):
        h0, c0, cache_t = lstm_step_forward(x[:, t, :], h0, c0, Wx, Wh, b)
        h[:, t, :] = h0
        cache.append(cache_t)
    ##############################################################################
    #                               END OF YOUR CODE                             #
