...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        loss += self.reg * sum(1 / 2 * (w ** 2).sum() for w in [self.params['W1'], self.params['W2']])
        dh, grads['W2'], grads['b2'] = affine_backward(dy, cache2)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i, v in enumerate(hidden_dims):
            self.params[f'W{i + 1}'] = weight_scale * np.random.randn(layer_input_dim, hidden_dims[i])
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            if self.normalization == 'batchnorm':
                out, norm_cache = batchnorm_forward(out, self.params[f'gamma{i}'],
                self.params[f'beta{i}'], self.bn_params[i - 1])
            elif self.normalization == 'layernorm':
                out, norm_cache = layernorm_forward(out, self.params[f'gamma{i}'],
                self.params[f'beta{i}'], self.bn_params[i - 1])
            else:
                norm_cache = None
        ...
        ############################################################################
        loss, dy = softmax_loss(scores, y)
        loss += self.reg * sum(1 / 2 * (v ** 2).sum() for k, v in self.params.items() if k.startswith('W'))
        dout, grads[f'W{self.num_layers}'], grads[f'b{self.num_layers}'] = affine_backward(dy, caches[-1])
        for i in range(self.num_layers - 1)[::-1]:
            if self.use_dropout:
                dout = dropout_backward(dout, caches[i][3])
            dout = relu_backward(dout, caches[i][2])
            if self.normalization == 'batchnorm':
                dout, grads[f'gamma{i + 1}'], grads[f'beta{i + 1}'] = batchnorm_backward_alt(dout, caches[i][1])
            elif self.normalization == 'layernorm':
                dout, grads[f'gamma{i + 1}'], grads[f'beta{i + 1}'] = layernorm_backward(dout, caches[i][1])
            dout, grads[f'W{i + 1}'], grads[f'b{i + 1}'] = affine_backward(dout, caches[i][0])
        for k in grads:
            if k.startswith('W'):
